...
export function generateApiClientStub(
  ...
  const lines = [
    ...
    '    }',
    '',
    '    // Extract just the data fields (skip state flags like isLoading, error)',
    '    const { isLoading, error, isOffline, lastSync, activeTab, ...dataFields } = merged',
    '    const arrays = Object.values(dataFields).filter((v: any) => Array.isArray(v))',
    '    if (arrays.length === 1) return arrays[0]  // Single array → return it directly',
    '    if (Object.keys(dataFields).length === 1) return Object.values(dataFields)[0]',
    '    return Object.keys(dataFields).length > 0 ? dataFields : merged',
    '  } catch {',
    '    return null',
    '  }',
    '}',
    ...
    '// eslint-disable-next-line @typescript-eslint/no-explicit-any',
    'const stub: any = {',
    '  get: () => Promise.resolve({ success: true, data: __getResponseData(), error: undefined }),',
    '  post: () => Promise.resolve({ success: true, data: __getResponseData(), error: undefined }),',
    '  put: () => Promise.resolve({ success: true, data: __getResponseData(), error: undefined }),',
    '  patch: () => Promise.resolve({ success: true, data: __getResponseData(), error: undefined }),',
    '  delete: () => Promise.resolve({ success: true, data: null, error: undefined }),',
    '  request: () => Promise.resolve({ success: true, data: __getResponseData(), error: undefined }),',
    '}',
    '',
  ]
